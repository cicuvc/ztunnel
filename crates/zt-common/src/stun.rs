...
pub fn build_binding_request() -> Vec<u8> {
    ...
    let tx_id: [u8; 12] = rand_transaction_id();
...
pub fn parse_xor_mapped_address(data: &[u8]) -> Result<XorMappedAddress, StunError> {
    ...
    if magic != STUN_MAGIC && msg_type != 0x0101 {
...
pub async fn discover_mapping(
    ...
    let addr = parse_xor_mapped_address(&buf[..n])?;
...
mod tests {
    ...
    fn test_parse_unknown_attribute_skipped() {
        // Unknown attribute (0x9999) before XOR-MAPPED-ADDRESS
        let data = build_stun_response(&[
            0x99,
            0x99, // unknown type
            0x00,
